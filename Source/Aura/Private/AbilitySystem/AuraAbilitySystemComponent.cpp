...
FGameplayTag UAuraAbilitySystemComponent::GetStatusForSpec(const FGameplayAbilitySpec& AbilitySpec)
{
...
void UAuraAbilitySystemComponent::UpDataAbilityStatuses(int32 Level)
{
	UAbilityInfo* AbilityInfo = UAuraAbilitySystemLibrary::GetAbilityInfo(GetAvatarActor());
	for (const FAuraAbilityInfo& Info : AbilityInfo->AuraAbilityInformation)
	{
		if (!Info.AbilityTag.IsValid()) continue;
		if (Level < Info.LevelRequirement) continue;

		// 指定のアビリティがアクティブでない場合アクティブにする
...
void UAuraAbilitySystemComponent::ServerSpendSpellPoint_Implementation(const FGameplayTag& AbilityTag)
{
	if (FGameplayAbilitySpec* AbilitySpec = GetSpecFromAbilityTag(AbilityTag))
	{
		...
		FGameplayTag Status = GetStatusForSpec(*AbilitySpec);
	...
	const FGameplayTag& Slot)
{
	if (FGameplayAbilitySpec* AbilitySpec = GetSpecFromAbilityTag(AbilityTag))
	{
		...
		const FGameplayTag& Status = GetStatusForSpec(*AbilitySpec);

		const bool bStatusValid = Status == GameplayTags.Abilities_Status_Equipped || Status == GameplayTags.Abilities_Status_Unlocked;
		if (bStatusValid)
		{
			// パッシブアビリティのアクティベートと非アクティベート
