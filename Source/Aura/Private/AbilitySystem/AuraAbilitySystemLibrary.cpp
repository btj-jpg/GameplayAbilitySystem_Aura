...
UDebuffInfo* UAuraAbilitySystemLibrary::GetDebuffInfo(const UObject* WorldContextObject)
{
	const AAuraGameModeBase* AuraGameMode = Cast<AAuraGameModeBase>(UGameplayStatics::GetGameMode(WorldContextObject));
	if (AuraGameMode == nullptr) return nullptr;
	return  AuraGameMode->DebuffInfo;
}
...
void UAuraAbilitySystemLibrary::SetIsRadiaolDamage(FGameplayEffectContextHandle& EffectContextHandle,
	bool bInIsRadiaolDamage)
{
	if (FAuraGameplayEffectContext* AuraEffectContext = static_cast<FAuraGameplayEffectContext*>(EffectContextHandle.Get()))
	{
		AuraEffectContext->SetRadialDamage(bInIsRadiaolDamage);
	}
}
...
TArray<FVector> UAuraAbilitySystemLibrary::EvenlyRotateVectors(const FVector& Forward, const FVector& Axis, float Spread, int32 NumRotators)
{
...
FGameplayEffectContextHandle UAuraAbilitySystemLibrary::ApplyDamageEffect(FDamageEffectParams Params)
{
	...
	SetIsRadiaolDamage(EffectContextHandle, Params.bIsRadialDamage);
